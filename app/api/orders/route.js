...
export async function POST(request){
    try {
        ...
        //check if requirred fields are present
...
//check if coupon is applicable for new user
        if(couponCode && coupon.forNewUser){
            ...
            if(userOrders.length > 0){
                return NextResponse.json({error: "Coupon is valid for new users"}, {status: 400});
            }
        }
        
        const isPlusMember = has({plan: 'plus'});

        //check if coupon is applicable for member
        ...
        return NextResponse.json({message : 'Orders Placed Succesfully'});
